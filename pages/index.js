...
export default function SovereignFinalUI() {
  ...
  const years = Array.from({ length: 40 }, (_, i) => 2026 - i); // من 2026 نزولاً
          ...
          <div style={{ background: "white", borderRadius: "20px", padding: "25px", boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
            ...
            {trainingData.steps[currentStep].fields && trainingData.steps[currentStep].fields.map(f => (
              <div key={f.id} style={{ marginBottom: "20px" }}>
                ...
                {f.type === 'strategic-date' ? (
                  /* منتقي التاريخ الثلاثي السيادي */
                  <div style={{ display: "flex", gap: "8px" }}>
                    <select style={{ flex: 1.2, padding: "12px", borderRadius: "10px", border: "2px solid #eee", fontSize: "14px", fontWeight: 700, appearance: "none", background: "#fff" }}>
                      <option>السنة</option>
                      {years.map(y => <option key={y} value={y}>{y}</option>)}
                    </select>
                    <select style={{ flex: 1.5, padding: "12px", borderRadius: "10px", border: "2px solid #eee", fontSize: "14px", fontWeight: 700, appearance: "none", background: "#fff" }}>
                      <option>الشهر</option>
                      {months.map((m, i) => <option key={i} value={i+1}>{m}</option>)}
                    </select>
                    <select style={{ flex: 1, padding: "12px", borderRadius: "10px", border: "2px solid #eee", fontSize: "14px", fontWeight: 700, appearance: "none", background: "#fff" }}>
                      <option>اليوم</option>
            ...
            {trainingData.steps[currentStep].questions && trainingData.steps[currentStep].questions.map(q => (
              <div key={q.id} style={{ marginBottom: "30px" }}>
                ...
                <textarea rows="4" style={{ width: "100%", padding: "15px", borderRadius: "10px", border: "2px solid #eee", boxSizing: "border-box" }} placeholder="اكتب التحليل الفني هنا..."></textarea>
              </div>
            ))}
